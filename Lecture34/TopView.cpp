...
void dfs(TreeNode<int>*root, map<int, int>&mp, int Column_No) {
	...
	//Because for every column we want to insert the first element of the column in map.
	mp.insert({Column_No, root->val});

	//If Updating , only do when the key is not present.
	// if (mp.count(Column_No) == 0) {
	// 	mp[Column_No] = root->val;
	// }

	// if (mp.find(Column_No) == mp.end()) {
	// 	mp[Column_No] = root->val;
	// }
